...
function productTaskCopy(settings = {}) {
  ...
  if (settings.exam === "TOPIK" && settings.level === "II") {
    return {
      ...
      writing: [
        taskCopy(`${topikLevel} 写作：趋势描述`, `围绕${target}练上升、下降、比较和总结`, ["完成本组写作结构题", "系统记录完成情况", "整理可复用句式", "完成后可补一句反思"]),
        taskCopy(`${topikLevel} 写作：图表开头句`, `围绕${target}练资料主题说明和开头模板`, ["完成本组写作结构题", "检查开头是否客观", "整理可复用句式", "完成后可补一句反思"]),
        taskCopy(`${topikLevel} 写作：观点理由展开`, `围绕${target}练观点、理由和例子`, ["完成本组写作结构题", "检查逻辑连接", "记录薄弱表达", "完成后可补一句反思"]),
  ...
  return {
    ...
    vocab: [
      taskCopy(`${topikLevel} 词汇语法：助词语尾基础`, `围绕${target}练基础句型和高频表达`, ["完成本组词汇语法题", "系统统计正确率", "错题进入复习队列", "完成后可补一句反思"])
    ],
    consolidation: [
      taskCopy(`${topikLevel} 巩固练习：阶段综合检验`, `围绕${target}做一组综合同型题`, ["完成本组系统练习", "系统统计正确率", "错题产生后进入错题集", "完成后可补一句反思"])
    ],
