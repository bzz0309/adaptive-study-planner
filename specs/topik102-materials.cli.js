async page => {
  ...
  const bankAudit = await page.evaluate(() => {
    ...
    const readingPassage = realMaterialQuestionsForContext({ settings, category: "reading", title: "短文大意理解" }, 5);
    ...
    return {
      ...
      readingPassage: readingPassage.map(item => ({ id: item.materialQuestionId, instruction: item.instruction, passage: item.passage, image: item.materialImage })),
  ...
  check("reading cohesion routes to verified questions 13-19 and 21", bankAudit.readingCohesion.length === 8 && bankAudit.readingCohesion.every(item => /^topik-ii-reading-102-q(?:01[3-9]|021)$/.test(item.id)), JSON.stringify(bankAudit.readingCohesion));
  ...
  check("short-passage reading routes only to verified questions 20 and 22", bankAudit.readingPassage.length === 2 && bankAudit.readingPassage.every(item => /^topik-ii-reading-102-q(?:020|022)$/.test(item.id)), JSON.stringify(bankAudit.readingPassage));
  check("short-passage reading keeps instruction and passage separate", bankAudit.readingPassage.every(item => item.instruction && item.passage && item.instruction !== item.passage && item.image?.includes("topik102-reading/question/")), JSON.stringify(bankAudit.readingPassage));
